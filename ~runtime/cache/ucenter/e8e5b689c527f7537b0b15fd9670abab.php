...
<div class="layui-tab">
  <ul class="layui-tab-title">
	 <!-- <li onclick="javascript:location.href='<?php echo U('Ucenter/Message/message',array('t'=>'unread'));?>'" 
	 <?php if(($t) == "unread"): ?>class="layui-this"<?php endif; ?>
	 >未读消息</li> -->
	 <li onclick="javascript:location.href='<?php echo U('Ucenter/Message/message',array('t'=>'all'));?>'" 
	 <?php if(($t) == "all"): ?>class="layui-this"<?php endif; ?>
	 >全部消息</li>
   <div style="float:right;"><a href="<?php echo U('Ucenter/Message/setmessageread');?>" class="layui-btn layui-btn-primary layui-btn-xs ajax-get confirm" style="display:inline-block">全部已读</a><a href="<?php echo U('Ucenter/Message/clearmessage');?>" class="layui-btn layui-btn-danger layui-btn-xs ajax-get confirm" style="display:inline-block">清空</a></div>
  </ul>
  <div class="layui-tab-content">
    <div class="layui-tab-item layui-show">
	<?php if(count($messages) == 0): ?><div class="title" style="font-size: 18px;line-height: 50px;">暂无消息～</div>
    <?php else: ?>
    <?php if(is_array($messages)): $i = 0; $__LIST__ = $messages;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$message): $mod = ($i % 2 );++$i;?><div class="qn-msg">
                <div class="message_icon">
				<?php if($message["from_uid"] == 1): ?>【平台推送】
				<?php else: ?>
                  【<?php echo ((isset($message["from_user"]["nickname"]) && ($message["from_user"]["nickname"] !== ""))?($message["from_user"]["nickname"]):'匿名'); ?>】<?php endif; ?>
				<div class="message-ctime"><?php echo (friendlydate($message["create_time"])); ?></div>
                </div>
                <dl class="message-dl">
                    <dt>
                        <h3 style="font-weight:bold"><?php if(($message["is_read"]) == "0"): ?>【未读】<?php endif; echo ($message["content"]["title"]); ?></h3>
                    </dt>
                    <?php if(($t) == "all"): ?><dd><?php echo ($message["content"]["content"]); ?></dd><?php endif; ?>
                </dl>
        </div><?php endforeach; endif; else: echo "" ;endif; ?>

	<?php echo getPagination($totalCount); endif; ?>
