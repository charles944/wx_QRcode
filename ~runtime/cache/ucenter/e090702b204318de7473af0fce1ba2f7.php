		...
		<div class="layui-tab">
			...
			<div class="layui-tab-content">
				<div class="layui-tab-item layui-show">
					<div class="title" style="font-size: 18px;line-height: 50px;">现有积分</div>
					<?php if(is_array($scores)): $i = 0; $__LIST__ = $scores;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><div class="qn-msg"><?php echo ($vo["title"]); ?>：<?php echo ($vo["value"]); ?> <?php echo ($vo["unit"]); ?></div><?php endforeach; endif; else: echo "" ;endif; ?>
				</div>
				<div class="layui-tab-item">
					<div class="title" style="font-size: 18px;line-height: 50px;">升级所需经验</div>
					<div class="qn-msg"><?php echo ($level); ?></div>
					<div class="qn-msg">
						当前经验：<?php echo ($self["score"]); ?>
					</div>
				</div>
				<div class="layui-tab-item">
					<?php if(is_array($action_module)): $i = 0; $__LIST__ = $action_module;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$m): $mod = ($i % 2 );++$i;?><div class="title" style="font-size: 18px;line-height: 50px;"><?php echo ((isset($m["module"]["alias"]) && ($m["module"]["alias"] !== ""))?($m["module"]["alias"]):'系统'); ?></div>
						<?php if(is_array($m["action"])): $i = 0; $__LIST__ = $m["action"];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i; if($vo['rule_array'] != false): ?><div class="qn-msg">
									<h4 class="list-group-item-heading"><strong><?php echo ($vo["title"]); ?>：</strong></h4>
									<?php if(is_array($vo["rule_array"])): $i = 0; $__LIST__ = $vo["rule_array"];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$r): $mod = ($i % 2 );++$i;?><p class="list-group-item-text">【<?php echo get_usergroups($r['usergroups']);?>】每次【<?php echo ($r["score"]["title"]); ?>】【<?php echo ($r["rule"]); ?>】<?php echo ($r["score"]["unit"]); ?>
											，每间隔【<?php echo ($r["cycle"]); ?>】<?php echo get_time_unit($r['time_unit']);?>，可最多【<?php echo ($r["max"]); ?>】次
