...
<div id="page">
	...
	<div class="ucenter-module-layout clearfix" id="ucenter-module-layout">
		<?php $mobiles = substr_replace($mobile,'****',3,4); ?>
		...
		<div class="layui-form layui-form-pane mine-view" <?php if($tabhash == 'bind'): ?>style="display:block"<?php else: ?>style="display:none"<?php endif; ?> >
			<form action="<?php echo U('Ucenter/Config/dobindmobile');?>" method="post" class="layui-form" role="form">
		...
		<div class="layui-form layui-form-pane mine-view" <?php if($tabhash == 'unbind'): ?>style="display:block"<?php else: ?>style="display:none"<?php endif; ?> >
			<form action="<?php echo U('Ucenter/Config/dounbindmobile');?>" method="post" class="form-horizontal form-without-legend ajax-form" role="form">
				<input type="hidden" class="form-control" name="type" value="mobile">
				<input type="hidden" class="form-control" name="uid" value="<?php echo is_login();?>">
				<div class="layui-form-item">
					<label for="" class="layui-form-label">已绑定手机号码</label>
					<div class="layui-input-inline">
						<input type="text" class="layui-input" name="num" placeholder="" value="<?php echo ($mobiles); ?>"/>
					</div>
    ...
    <script>
        ...
        $(function(){
            ...
            $button.click(function(){
				...
				$.post("<?php echo U('home/verify/sendVerify');?>", {account: account, type: 'mobile' ,action:'config', todo: 'bindmobile'}, function (res) {
					if(res.status){
						DecTime.obj = $this
            ...
            $button2.click(function(){
				$button.addClass('disabled',true);
				var $this = $(this);
				$.post("<?php echo U('home/verify/sendVerify');?>", {account: '<?php echo $mobile; ?>', type: 'mobile' ,action:'config', todo: 'unbindmobile'}, function (res) {
					if(res.status){
						DecTime.obj = $this
						DecTime.time = "<?php echo modC('SMS_RESEND','60','USERCONFIG');?>";
						$this.attr('disabled',true)
