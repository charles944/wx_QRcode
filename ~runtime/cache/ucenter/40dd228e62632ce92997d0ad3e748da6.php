...
<div class="layui-tab">
  <ul class="layui-tab-title">
    <li onclick="javascript:location.href='<?php echo U('System/useraccount/');?>'" <?php if(($tab_s) == "0"): ?>class="layui-this"<?php endif; ?>>总收入明细</li>
  </ul>
  <div class="layui-tab-content">
    <div class="layui-tab-item layui-show">
		<blockquote class="layui-elem-quote layui-quote-nm">
			<?php if(is_array($scores)): $i = 0; $__LIST__ = $scores;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><p><b><?php echo ($vo["title"]); ?></b>：<b style="color:red;font-size:16px"><?php echo (number_format($vo["value"],2)); ?></b> <?php echo ($vo["unit"]); ?></p><?php endforeach; endif; else: echo "" ;endif; ?>
		</blockquote>
		...
		<table class="layui-table">
			<thead>
				<tr>
					...
					<th width="180">明细</th>
					<?php if(is_array($scoretypelist)): $i = 0; $__LIST__ = $scoretypelist;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($i % 2 );++$i;?><th>+<?php echo getScoreTypeName($v['mark']);?></th><?php endforeach; endif; else: echo "" ;endif; ?>
					<?php if(is_array($scoretypelist)): $i = 0; $__LIST__ = $scoretypelist;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($i % 2 );++$i;?><th>=<?php echo getScoreTypeName($v['mark']);?></th><?php endforeach; endif; else: echo "" ;endif; ?>
				</tr>
			</thead>
			<tbody id="task_log_list">
			<?php if(!empty($accountdetail)): if(is_array($accountdetail)): $i = 0; $__LIST__ = $accountdetail;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
				<td><?php echo (date('Y-m-d H:i:s',$vo["create_time"])); ?></td>
				<td><a style="width:180px; height:20px; line-height:20px;display:inline-block; text-overflow:ellipsis; white-space:nowrap;overflow:hidden;" data-toggle="tooltip" title="" data-original-title="<?php echo (op_t($vo["remark"])); ?>"><?php echo (op_t($vo["remark"])); ?></a></td>
				<?php if(is_array($scoretypelist)): $i = 0; $__LIST__ = $scoretypelist;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($i % 2 );++$i;?><td><?php echo (number_format($vo[$v['mark']],0)); ?></td><?php endforeach; endif; else: echo "" ;endif; ?>
				<?php if(is_array($scoretypelist)): $i = 0; $__LIST__ = $scoretypelist;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($i % 2 );++$i; $mark = 're_'.$v['mark']; ?>
				<td><?php echo (number_format($vo[$mark],0)); ?></td>
				<?php unset($mark); endforeach; endif; else: echo "" ;endif; ?>
			</tr><?php endforeach; endif; else: echo "" ;endif; ?>
			<?php else: ?>
			<tr><td colspan="12">暂无获取明细</td></tr><?php endif; ?>
			</tbody>
		</table>
		<div class="home_page" id="page_content">
		<?php echo getPagination($totalPageCount,$r);?>
		</div>
