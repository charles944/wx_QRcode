...
<div class="layui-card qn-header">
	<div class="layui-breadcrumb">
		<a href="">首页</a>
		<a href="<?php echo U('index');?>"><cite>前台导航管理</cite></a>
			...
			<table class="layui-table" lay-even lay-skin="line">
				...
				<tbody>
					<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$channel): $mod = ($i % 2 );++$i;?><tr>
						...
						<td>
							<a href="<?php echo U('setStatus?ids='.$channel['id'].'&status='.abs(1-$channel['status']));?>" class="ajax-get layui-btn layui-btn-xs hbutton btn_see"><i class="layui-icon">&#x1006;</i> <?php echo (show_status_op($channel["status"])); ?></a>
							<a title="编辑" class="layui-btn layui-btn-xs hbutton btn_edit" href="<?php echo U('edit?id='.$channel['id'].'&pid='.$pid);?>"><i class="layui-icon">&#xe642;</i> 编辑</a>
							<a class="ajax-get layui-btn layui-btn-xs hbutton btn_trash" title="删除" href="<?php echo U('del?id='.$channel['id']);?>"><i class="layui-icon">&#xe640;</i> 删除</a>
						</td>
