			  ...
			  <div class="layui-tab-content">
				<div class="layui-tab-item layui-show">
					<div class="portlet-title">
						<div class="actions">
							<a href="<?php echo U('Interface/create');?>" class="layui-btn layui-btn-xs fbutton">新增</a>
							<a href="<?php echo U('Interface/log');?>" class="layui-btn layui-btn-xs fbutton"><i class="fa fa-bars"></i> 回调记录</a>
						</div>
						<div class="clear"></div>
					</div>
						...
						<table class="layui-table" lay-even lay-skin="line">
							...
							<tbody>
								<?php if(is_array($_list)): $i = 0; $__LIST__ = $_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
										...
										<td><?php echo ($vo["websiteid"]); ?></td>
										<td><?php echo ($vo["key"]); ?></td>
										<td>
										<?php switch($vo["review"]): case "0": ?>不审核<?php break;?>
										<?php case "1": ?>审核<?php break;?>
										<?php default: ?>不审核<?php endswitch;?>
										</td>
										<td>
										<?php switch($vo["status"]): case "0": ?>未安装<?php break;?>
										<?php case "1": ?>正常<?php break;?>
										<?php case "2": ?>冻结<?php break;?>
										<?php default: ?>正常<?php endswitch;?>
										</td>
										<td><?php echo ($vo["author"]); ?></td>
										<td><?php echo ($vo["version"]); ?></td>
										<td>
											<?php if(empty($vo["uninstall"])): ?><a class="layui-btn layui-btn-xs hbutton btn_see" href="<?php echo U('config',array('id'=>$vo['id']));?>">设置</a>
											<?php if ($vo['status'] >=0): ?>
												<?php if(($vo["status"]) == "0"): ?><a class="ajax-get layui-btn layui-btn-xs hbutton btn_see" href="<?php echo U('enable',array('id'=>$vo['id']));?>">启用</a>
												<?php else: ?>
													<a class="ajax-get layui-btn layui-btn-xs hbutton btn_trash" href="<?php echo U('disable',array('id'=>$vo['id']));?>">禁用</a><?php endif; ?>
											<?php endif ?>
											<a class="ajax-get layui-btn layui-btn-xs hbutton btn_trash" href="<?php echo U('del?ids='.$vo['id']);?>">卸载</a>
											<?php else: ?>
												<a class="ajax-get layui-btn layui-btn-xs hbutton btn_see" href="<?php echo U('install?name='.$vo['name']);?>">安装</a><?php endif; ?>
										</td>
