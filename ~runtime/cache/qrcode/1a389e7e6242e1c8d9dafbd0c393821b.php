...
<div class="layui-fluid">
	<div class="layui-card">
		<div class="layui-card-body">
			<div class="" style="margin-bottom:10px"><a href="<?php echo U('qrcode/index/edit');?>" class="layui-btn-sm layui-btn">新增活码</a></div>
				...
				<?php if(!empty($list)): if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$data): $mod = ($i % 2 );++$i;?><tr>
						...
						<td>
							<div style="text-align:center">
								<a href="<?php echo U('qrcode/index/qr',array('id'=>$data['id']));?>" target="_blank" style="border:none;"><img class="qrcode" src="/uploads/picture/QRcode/<?php echo ($data["id"]); ?>_<?php echo ($data["qr_domain"]); ?>.png?t=<?php echo TIME();?>"></a>
								<p style="text-align:center"><?php echo ($data["title"]); ?></p>
							</div>
							<div style="text-align:center">
								<a class="layui-btn-xs layui-btn" href="<?php echo U('qrcode/index/edit',array('id'=>$data['id']));?>">编辑活码</a>
							</div>
						</td>
						<td>
							<p >
								子码数量：<?php echo ($data["qrcode_child"]); ?>
							</p>
							<p >
								显示模式：<?php switch($data["view_mode"]): case "1": ?>随机显示<?php break; case "2": ?>逐个显示<?php break; endswitch;?>
							</p>
							<p >
								活码类型：<font color="red"><?php switch($data["type_mode"]): case "1": ?>微信群活码<?php break; case "2": ?>城市分组码<?php break; case "3": ?>自定义分组码<?php break; case "4": ?>客服循环码<?php break; case "5": ?>机型码<?php break; endswitch;?></font>
							</p>
							<p >
								每码阀值：<?php echo ($data["max_scan"]); ?>
							</p>
							<p>显示限制：<?php switch($data["view_limit_mode"]): case "1": ?>每刷新都不一样<?php break; case "2": ?>同IP显示一个<?php break; endswitch;?></p>
						</td>
						<td><a href="<?php echo U('qrcode/index/tjlog',array('id'=>$data['id']));?>">共 <?php echo ($data["today_people"]); ?> 人</a></td>
						<td><a href="<?php echo U('qrcode/index/log',array('id'=>$data['id']));?>">共 <?php echo ($data["scan_count"]); ?> 次</a></td>
						<td>共 <?php echo ($data["scan_child_count"]); ?> 次</td>
						<td>
							<a class="layui-btn-xs layui-btn" href="<?php echo U('qrcode/index/child',array('id'=>$data['id']));?>">子码管理</a>
							<a class="layui-btn-xs layui-btn layui-btn-danger ajax-get confirm" href="<?php echo U('qrcode/index/del',array('id'=>$data['id']));?>" >删除活码</a>
						</td>
			...
			<div class='layui-table-page'><?php echo getPagination($totalCount,$r);?></div>
			<div class="clear"></div>
		</div>
	</div>
</div>
