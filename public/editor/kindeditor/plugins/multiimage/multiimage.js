...
K.extend(KSWFUpload, {
	init : function(options) {
		...
		var settings = {
			...
			upload_success_handler : function(file, serverData) {
				...
				file.url = data.url.split('|');
				
				K('.ke-img', itemDiv).attr('src', file.url[0]).attr('data-status', file.filestatus).data('data', data);
				K('.ke-status > div', itemDiv).hide();
			}
		};
	...
	appendFile : function(file) {
		...
		var img = K('<img src="' + urls[0] + '" class="ke-img" data-status="' + file.filestatus + '" width="80" height="80" alt="' + file.name + '" />');
		photoDiv.append(img);
