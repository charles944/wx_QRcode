...
hui.loadMore = function(selector, func, title, loading){
	...
	var dom = hui(selector).find('.hui-load-more'), winInfo = hui.winInfo();
	if(dom.length < 1){
		dom = document.createElement('div');
		dom.setAttribute('class', 'hui-load-more');
		dom.innerHTML = title;
		if(hui(selector).length >= 1){
			hui(dom).prependTo(selector);
		}else{
			document.body.appendChild(dom);
		}
		dom = hui(selector).find('.hui-load-more');
	}
...
hui.endLoadMore = function(selector, isEnd, endMsg){
	if(!endMsg){endMsg = '已经加载全部';}
	var dom = hui(selector).find('.hui-load-more');
	if(isEnd){dom.html(endMsg); hui.loadMoreEnd = true;}
	hui.refreshIng = false;
}
hui.resetLoadMore = function(selector){
	hui.loadMoreEnd = false;
	hui(selector).find('.hui-load-more').html(hui.loadMoreText);
}