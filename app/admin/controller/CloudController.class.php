...
class CloudController extends AdminController
{
    ...
    public function getFileList()
    {
        ...
        if ($aVersion != $nextVersion['number']) {
        ...
        $this->downloadFile(appstoreU('Appstore/Update/download', array('number' => $nextVersion['number'], 'type' => 'new')), C('UPDATE_PATH') . $nextVersion['name'] . '/new.zip');
...
str
    ...
    public function version()
    {
        ...
        $versionModel = D('Admin/Version');
        $version = $versionModel->where(array('number' => $aName))->find();
    ...
    private function assignUpdatingGoods($goods)
    {
        ...
        switch ($goods['category']) {
            case 'addons':
				$module = D('Admin/Addons')->getAddons($goods['adtype']);
                $module = $cloudModel->getVersionInfo($module);
                $this->assign('modules', array($module));
                break;
            case 'app':
                $module = D('Common/Module')->getModule($goods['adtype']);
                $module = $cloudModel->getVersionInfo($module);
                $this->assign('modules', array($module));
                break;
            case 'themes':
    ...
    public function updateGoods()
    {
        ...
        $aToken = I('get.token', '', 'text');
		$oid = I('get.oid', '', 'text');
		$amd5 = I('get.md5', 0, 'text');
		if(empty($oid)){
			$this->error('参数错误，请检查version.json内容');
		}
		if(empty($amd5)){
			$this->error('参数错误，请检查version.json内容');
		}
        ...
        $versionList = D('Admin/Cloud')->getUpdateList($aToken, $oid, $amd5);
        $this->assign('versionList', $versionList);
        $this->assign('token', $aToken);
        $this->assign('version', $version);
    ...
    private function switchDir($entity)
    {
        switch ($entity) {
            case 'addons':
                return QN_ADDON_PATH;
            case 'app':
                return APP_PATH;
            case 'themes':
                return QN_THEME_PATH;
        }
    }
    ...
    {
        $this->meta_title = '应用自动升级 - 下载文件';

        //初始化各类信息
        $version = $_SESSION['version'];
		//array_reverse($_SESSION['versionList']);
        $versionList = $_SESSION['versionList'];
        $token = $_SESSION['token'];
		$oid = $_SESSION['oid'];
		$md5 = $_SESSION['md5'];
		$version_id = I('get.version_id','','text');
        $this->assign('version', $version);
        $this->assign('versionList', $versionList);
        if (empty($version)) {
        ...
        $path = C('CLOUD_PATH') . $version['goods']['category'] . '/' . $version['goods']['adtype'] . '/' . $versionList[0]['version'];
        ...
        //下载文件
        $this->write('开始下载原版' . $version['goods']['version'] . '文件。', 'info');
        $this->downloadFile(appstoreU('Appstore/Install/download', array('token' => $token, 'md5'=>$md5, 'oid'=>$oid, 'type' => 'current')), $path . '/old.zip');
        $this->write('开始下载新版' . $versionList[0]['version'] . '文件。', 'info');
        $this->downloadFile(appstoreU('Appstore/Install/download', array('token' => $token, 'md5'=>$md5, 'oid'=>$oid, 'new_version'=>$version_id, 'type' => 'next')), $path . '/new.zip');
    ...
    public function updating2()
    {
        ...
        $compared_with_old = $this->diff($old_file_path, $this->switchDir($version['goods']['category']), array($version['goods']['adtype'] . '/update.sql' => array('db', 1)));
        $compared_with_new = $this->diff($new_file_path, $this->switchDir($version['goods']['category']), array($version['goods']['adtype'] . '/update.sql' => array('db', 1)));
        $compared = $compared_with_old + $compared_with_new;
        $this->assign('path', $path);
        $this->assign('compared', $compared);
        $this->disableCheckUpdate();
...
str
    ...
    public function updating3()
    {
        ...
        $this->assign('backup_path', $backup_path);
        $need_back = $this->treeDirectory($old_file_path, $old_file_path);
        $this->disableCheckUpdate();
        $this->display();
        ...
        foreach ($need_back as $v) {
            ...
            $from = realpath($this->switchDir($version['goods']['category']) . '/' . text($v));
            ...
            if (file_exists($des) === false) {
                ...
                $this->write(str_replace(array('\\', '//'), array('\\\\', '/'), '备份文件 ' . $this->switchDir($version['goods']['category']) . text($v) . ' 到 ' . str_replace('./', '', $backup_path) . text($v)) . '……成功', 'success');
            }
        }
        $this->write('文件全部备份完成。');
        //覆盖文件
        ...
        foreach ($need_back as $v) {
            ...
            $from = realpath($new_file_path . text($v));
            $des = str_replace(array('/', '.' . DIRECTORY_SEPARATOR, DIRECTORY_SEPARATOR . DIRECTORY_SEPARATOR), array(DIRECTORY_SEPARATOR, DIRECTORY_SEPARATOR, DIRECTORY_SEPARATOR), dirname(realpath('./index.php')) . $this->switchDir($version['goods']['category']) . text($v));
            $des_dir = substr($des, 0, strrpos($des, DIRECTORY_SEPARATOR));
            if (!is_dir($des_dir)) {
                $this->createFolder($des_dir);
            }
    ...
    public function updating4()
    {
        ...
        $path = C('CLOUD_PATH') . $version['goods']['category'] . '/' . $version['goods']['adtype'] . '/' . $versionList[0]['version'];
        $pathOld = $path . '/old';
        $pathNew = $path . '/new' . '/' . $version['goods']['adtype'] . '/';
    ...
    {
		$this->meta_title = '应用自动升级 - 完成设置版本号和token';
        $version = $_SESSION['version'];
        $versionList = $_SESSION['versionList'];
        $this->assign('version', $version);
        $this->assign('versionList', $versionList);
        $newToken = $versionList[0]['version_token'];
        switch ($version['goods']['category']) {
            case "addons":
                $addonsModel = D('Admin/Addons');
				$addonsModel->setToken($version['goods']['adtype'], $newToken);
				$addonsModel->updatelist();
                break;
            case "app":
                $moduleModel = D('Common/Module');
                $moduleModel->setToken($version['goods']['adtype'], $newToken);
                $moduleModel->reloadModule($version['goods']['adtype']);
                //$_SESSION['version']= $versionList[0];
                $moduleModel->cleanModulesCache();
                break;
            case "themes":
				$themeModel = D('Common/Theme');
                $themeModel->setToken($version['goods']['adtype'], $newToken);
                break;
        }
    ...
    public function install()
    {
		...
		$aMd5 = I('post.md5','','text');
        $aCookie = I('post.cookie', '', 'text');
		$aOid = I('post.oid','','text');
		
        $_SESSION['cloud_cookie'] = $aCookie;
        $this->display();
        set_time_limit(0);
        $this->write('自动安装应用开始......<br/>开始获取版本信息......', 'info');
        $this->write('&nbsp;&nbsp;&nbsp;>连接远程服务器....', 'info');
        $data = $this->curl($this->url(cloudU('Appstore/Install/getVersion', array('token' => $aToken,'md5'=>$aMd5,'oid'=>$aOid))));
		//$data = file_get_contents($this->url(cloudU('Appstore/Install/getVersion', array('token' => $aToken,'version'=>$aMd5,'oid'=>$aOid))));
        ...
        if (!$data['status']) {	
            $this->write('解析服务器返回结果失败。安装意外终止。' . $data['msg'], 'danger');
        }
        $version = $data['version'];
        switch (strtolower($version['goods']['category'])) {
            case "addons":
                $this->installPlugin($version, $aToken, $aMd5, $aOid);
                break;
            case "app":
                $this->installModule($version, $aToken, $aMd5, $aOid);
                break;
            case "themes":
                $this->installTheme($version, $aToken, $aMd5, $aOid);
                break;
        }
    }

    private function installPlugin($version, $token, $aMd5, $aOid)
    {
        $plugin['name'] = ucfirst(strtolower($version['goods']['adtype']));
        $plugin['alias'] = $version['goods']['title'];
		$plugin['version'] = $version['goods']['version'];
        $this->write('&nbsp;&nbsp;&nbsp;>当前正在安装的是【插件】，插件名【' . $plugin['alias'] . '】【' . $plugin['name'] . '】');
        if (file_exists(QN_ADDON_PATH . '/' . $plugin['name'])) {
            //todo 进行版本检测
        ...
        //下载文件
		$localPath = C('CLOUD_PATH') . strtolower($version['goods']['category']) . '/' ;
		$this->createFolder($localPath);
        $localFile = $localPath . $plugin['name'].' V'. $plugin['version'] . '.zip';
        $this->downloadFile($this->url(cloudU('Appstore/Install/download', array('token' => $token,'md5'=>$aMd5,'oid'=>$aOid))), $localFile);
        chmod($localFile, 0777);
        //开始安装
        $this->write('开始安装插件......');
        $this->unzipFile($localFile, QN_ADDON_PATH);
        $rs = D('Addons')->install($plugin['name']);
        if ($rs === true) {

            $tokenFile=QN_ADDON_PATH . $plugin['name'] . '/version.json';
			$tokenmd5 = json_encode(array('token'=>$token, 'oid'=>$aOid, 'md5'=>$aMd5));
            if(file_put_contents($tokenFile,$tokenmd5)){
...
str
            ...
            $this->write('&nbsp;&nbsp;&nbsp;>插件安装失败。'. D('Addons')->getError(), 'danger');
        }
    ...
    private function installTheme($version, $token, $aMd5, $aOid)
    {
        $theme['name'] = ucfirst(strtolower($version['goods']['adtype']));
        $theme['alias'] = $version['goods']['title'];
		$theme['version'] = $version['goods']['version'];
        $this->write('&nbsp;&nbsp;&nbsp;>当前正在安装的是【主题】，主题名【' . $theme['alias'] . '】【' . $theme['name'] . '】');
        if (file_exists(QN_THEME_PATH . $theme['name'])) {
            //todo 进行版本检测
		...
		$localPath = C('CLOUD_PATH') . strtolower($version['goods']['category']) . '/' ;
		$this->createFolder($localPath);
        $localFile = $localPath . $theme['name'].' V'. $theme['version'] . '.zip';
        $this->downloadFile($this->url(cloudU('Appstore/Install/download', array('token' => $token,'md5'=>$aMd5,'oid'=>$aOid))), $localFile);
        chmod($localFile, 0777);
        //开始安装
       $this->unzipFile($localFile,QN_THEME_PATH);
        ...
        if ($res === true) {
            $tokenFile=QN_THEME_PATH.$theme['name'].'/version.json';
			$tokenmd5 = json_encode(array('token'=>$token, 'oid'=>$aOid, 'md5'=>$aMd5));
            if(file_put_contents($tokenFile,$tokenmd5)){
                $this->write('&nbsp;&nbsp;&nbsp;>主题安装成功。即将跳转到本地主题页面。', 'success');
                $jump = U('Theme/index', array('cleanCookie' => 1));
                sleep(2);
                $this->writeScript(<<<str
        location.href="$jump";
str
    ...
    private function installModule($version, $token, $aMd5, $aOid)
    {
        $module['name'] = ucfirst(strtolower($version['goods']['adtype']));
        $module['alias'] = $version['goods']['title'];
		$module['version'] = $version['goods']['version'];
        $this->write('&nbsp;&nbsp;&nbsp;>当前正在安装的是【模块】，模块名【' . $module['alias'] . '】【' . $module['name'] . '】');
        if (file_exists(APP_PATH . $module['name'])) {
            //todo 进行版本检测
		...
		$localPath = C('CLOUD_PATH') . strtolower($version['goods']['category']) . '/' ;
		$this->createFolder($localPath);
        $localFile = $localPath . $module['name'].' V'. $module['version'] . '.zip';
        $this->downloadFile($this->url(cloudU('Appstore/Install/download', array('token' => $token,'md5'=>$aMd5,'oid'=>$aOid))), $localFile);
        //chmod($localFile, 0777);
        //开始安装
        ...
        if ($res === true) {
            ...
            $tokenFile= APP_PATH  . $module['name'] . '/Info/version.json';
			$tokenmd5 = json_encode(array('token'=>$token, 'oid'=>$aOid, 'md5'=>$aMd5));
            $this->cleanModuleListCache();
           if($moduleModel->setToken($module['name'],$tokenmd5)){
