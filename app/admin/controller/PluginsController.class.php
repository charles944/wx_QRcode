...
class PluginsController extends AdminController 
{
    ...
    public function index()
    {
        ...
        if ($type == 'yes') {//已安装的
            foreach ($list as $key => $value) {
            	$list[$key]['path'] = ltrim(QN_PLUGIN_PATH,'.'). strtolower($value['name']);
    ...
    public function adminList($name, $method = null)
    {
		...
		if(empty($method)){
			if (method_exists(A('plugins://' . $name . '/Admin'), 'buildList')) {
				A('plugins://' . $name . '/Admin')->buildList();
				exit;
			}
		}else{
			if (method_exists(A('plugins://' . $name . '/Admin'), $method)) {
				A('plugins://' . $name . '/Admin')->$method();
				exit;
			}
		}
        ...
        if (!isset($map))
            $map = array();
        if (isset($model))
            $list = $this->lists(D("Plugins://{$model}/{$model}")->field($fields), $map, $order);
    ...
    public function config(){
        if(IS_POST){
            $id = (int)I('id');
            $config = I('config');
            $flag = M('Plugins')->where("id={$id}")->setField('config', json_encode($config));
