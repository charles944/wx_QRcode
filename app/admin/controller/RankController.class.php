...
class RankController extends AdminController
{
    ...
    public function userlist()
    {
        ...
        $model = D('User/Member');
        $rankList = $model->where($map)->page($page, 20)->select();
        ...
        $builder
        ->title('用户头衔列表')
        ->buttonNew(U('rank/editrank'), '新增头衔')
		->Button('启 用',array('class'=>'layui-btn layui-btn-xs fbutton ajax-post','href'=>U('setstatus',array('status'=>1)),'target-form'=>'ids'))
		->Button('禁 用',array('class'=>'layui-btn layui-btn-xs fbutton ajax-post','href'=>U('setstatus',array('status'=>0)),'target-form'=>'ids'))
		->buttonDelete(U('setStatus',array('status'=>-1)),'删除')
        ->keyText('uid', 'UID')
		->keyText('nickname', '昵称')
		->keyStatus()
		->keyDoAction('Rank/userRankList?id=[uid]', '用户头衔列表')
		->keyDoAction('Rank/userAddRank?id=[uid]', '关联新头衔')
        ->data($rankList)
    ...
    public function userRankList($id = null, $page = 1)
    {
        ...
        $builder
			...
			->keyText('title', '头衔名称')
			->keyText('reason', '颁发理由')
			->keyText('is_show', '是否显示在昵称右侧')
			->keyCreateTime()
			->keyDoActionEdit('Rank/userChangeRank?id=###')
			->keyDoAction('Rank/deleteUserRank?id=###', '删除')
            ->data($rankList)
            ->pagination($totalCount, 20)
            ->display();
    }
    ...
    {
        D('Common/Message')->sendMessage($data['uid'], $type, $content, 'Ucenter/Message/message',array(),is_login(), 1);
    }
    ...
    public function rankVerify($page = 1)
    {
        ...
        $builder
            ->title('待审核的头衔列表')
            ->buttonSetStatus(U('setVerifyStatus'), '1', '审核通过', array('class'=>'layui-btn layui-btn-xs fbutton green ajax-post'))->buttonDelete(U('setVerifyStatus'), '审核不通过')
    ...
    public function rankVerifyFailure($page = 1)
    {
        ...
        $builder
            ->title('被驳回的头衔申请列表')
            ->buttonSetStatus(U('setVerifyStatus'), '1', '审核通过', array('class'=>'layui-btn layui-btn-xs fbutton ajax-post'))
            ->keyId()->keyText('uid', '用户ID')->keyText('u_name', '用户昵称')->keyText('title', '头衔名称')->keyText('reason', '申请理由')->keyText('is_show', '是否显示在昵称右侧')->keyCreateTime()->keyDoActionEdit('Rank/userChangeRank?id=###')
            ->data($rankList)
            ->pagination($totalCount, 20)
            ->display();
