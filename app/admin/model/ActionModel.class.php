...
class ActionModel extends Model {
    ...
    public function update(){
        ...
        for($i=0;$i<count($action_rule['table']);$i++){
            $_POST['rule'][] = array('table'=>$action_rule['table'][$i],'field'=>$action_rule['field'][$i],'rule'=>$action_rule['rule'][$i],'cycle'=>$action_rule['cycle'][$i],'time_unit'=>$action_rule['time_unit'][$i],'max'=>$action_rule['max'][$i],'usergroups'=>$action_rule['usergroups'][$i]);
        }
        if(empty($_POST['rule'])){
            $_POST['rule'] ='';
        }else{
