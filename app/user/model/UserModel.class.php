...
class UserModel
{
    private $member_fields = array('uid', 'nickname', 'sex', 'birthday', 'qq', 'login', 'reg_ip', 'reg_time', 'last_login_ip', 'last_login_time', 'status', 'last_login_role', 'show_role', 'signature', 'pos_province', 'pos_city', 'pos_district', 'pos_community', 'score', 'jifenbao', 'money', 'gold_coin', 'con_check', 'total_check');
    ...
    function query_user($fields = null, $uid = null)
    {
        ...
        if(isset($result['avatar64'])){
            $result['avatar64'] = '/' . ltrim($result['avatar64'], '/');
        }
        if(isset($result['avatar32'])){
            $result['avatar32'] = '/' . ltrim($result['avatar32'], '/');
        }
        if(isset($result['avatar128'])){
            $result['avatar128'] = '/' . ltrim($result['avatar128'], '/');
        }
        //返回结果
        return $result;
    }
    ...
    public function getAvatars($uid, $avatarFields)
    {
		//读取头像数据
        $result = array();
        $avatarObject = new \home\widget\UploadAvatarWidget();
