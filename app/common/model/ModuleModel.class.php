...
class ModuleModel extends Model
{
    ...
    protected $tokenFile = '/info/version.json';
        ...
        if ($module === false) {
            ...
            foreach ($dir as $subdir) {
                if (file_exists(APP_PATH . '/' . $subdir . '/info/info.php') && $subdir != '.' && $subdir != '..') {
                    ...
                    //如果token存在的话
					$token_arr = $this->getToken($this->moduleName);
                    if (!empty($token_arr)) {
						$info['token'] = $token_arr['token'];
						$info['oid'] = $token_arr['oid'];
						$info['md5'] = $token_arr['md5'];
						unset($token_arr);
                    }
                    $module[] = $info;
                }
    ...
    public function checkCanVisit($name)
    {
        $modules = $this->getAll();
        foreach ($modules as $m) {
            if (isset($m['is_setup']) && $m['is_setup'] == 0 && strtolower($m['name']) == strtolower($name)) {
                header("Content-Type: text/html; charset=utf-8");
                exit('你所访问的模块未安装，禁止访问!');
            }
        }
    ...
    public function getModule($name)
    {
        ...
        if ($module === false || $module == null) {
            ...
            if ($m != array()) {
                ...
                $m['id'] = $this->add($m);
				$token_arr = $this->getToken($m['name']);
				if (!empty($token_arr)) {
			...
			$token_arr = $this->getToken($module['name']);
			if (!empty($token_arr)) {
				$module['token'] = $token_arr['token'];
				$module['oid'] = $token_arr['oid'];
				$module['md5'] = $token_arr['md5'];
				unset($token_arr);
			}
            return $module;
        }
