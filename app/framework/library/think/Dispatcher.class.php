...
class Dispatcher {
    ...
    static public function dispatch() {
        ...
        if(empty($_SERVER['PATH_INFO'])) {
			...
			// add with 2016.2.25 by 朝兮夕兮 修改支持 index.php/addon/Redbag/Redbag/lists 这样的插件短地址
			...
			// add with 2016.2.25 by 朝兮夕兮 end
		...
		/**加载主题公共配置文件，增加主题配置文件加载 start */
		if(is_file(QN_THEME_PATH.'config.php')){
			$TMPL_PARSE_STRING=C('TMPL_PARSE_STRING');
			C(load_config(QN_THEME_PATH.'config.php'));
    ...
    static private function getModule($var) {
        ...
        if($maps = C('URL_MODULE_MAP')) {
            if(isset($maps[strtolower($module)])) {
				...
				return strtolower($maps[MODULE_ALIAS]);
