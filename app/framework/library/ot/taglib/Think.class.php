...
class Think extends TagLib{
    ...
    protected $tags   =  array(
    	...
    	'usernav'   =>  array('attr' => 'field,name', 'close' => 1), //获取会员中心导航
    ...
    public function _nav($tag, $content){
        ...
        $parse  = $parse   = '<?php ';
        $parse .= '$__NAV__ = D(\'Common/Channel\')->lists('.$field.');';
        if($tree){
    ...
    /* 导航列表 */
    public function _usernav($tag, $content){
    	$field  = empty($tag['field']) ? 'true' : $tag['field'];
    	$tree   =   empty($tag['tree'])? false : true;
    	$parse  = $parse   = '<?php ';
    	$parse .= '$__NAV__ = M(\'UserChannel\')->field('.$field.')->where("status=1")->order("sort")->select();';
    	if($tree){
    		$parse .= '$__NAV__ = list_to_tree($__NAV__, "id", "pid", "_");';
    	}
    	$parse .= '?><volist name="__NAV__" id="'. $tag['name'] .'">';
